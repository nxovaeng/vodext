...
class AdFilterInterceptor : Interceptor {
    ...
    private fun shouldPreserveLine(line: String): Boolean {
        return line.startsWith("#EXTM3U") ||
               ...
               line.isEmpty() ||
               (line.startsWith("#") && !line.startsWith("#EXTINF") && !line.startsWith("#EXT-X-DISCONTINUITY"))
    }
    ...
    private fun isAdSegment(
        ...
        if (statistics.isDurationAbnormal(duration)) {
            adScore += 20
            if (DEBUG) Log.d(TAG, "Duration abnormal: $duration vs avg ${statistics.averageDuration}")
        }
    ...
    private fun containsAdKeywords(url: String): Boolean {
        val adKeywords = listOf(
            "ad", "ads", "adv", "advertisement", "adjump",
            "sponsor", "commercial", "promo", "promotion",
            "tracking", "adserver", "doubleclick",
            "p1ayer"  // 混淆的 player
        )
     ...
     */
    private val AD_FILTER_RULES = listOf(
        // ========== 暴风资源 (bfzy) ==========
        AdFilterRule(
            name = "暴风",
        ...
        AdFilterRule(
            name = "非凡",
            hosts = listOf("vip.ffzy", "hd.ffzy", "super.ffzy", "svipsvip.ffzy", ".ffzy"),
            regex = listOf(
